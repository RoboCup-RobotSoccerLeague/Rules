...
\subsection{Division and complexity level}
\label{sec:divisions}

A team belongs to one division, which is one of Small, Mid and Large. Main-competition matches only take place between members in the same division.

A team also has one complexity level, which is either Foundation or Advanced. Teams of different levels may play each other: if at least one team in the match is Foundation, then the match is Foundation, otherwise it is Advanced. (As of this draft version, the complexity level only affects the maximum team size.)

Both the division and the complexity level of a team are defined before the competition (precise modality TBD) and cannot change during the competition itself.
...
A match is played by two teams, each with a \textbf{maximum} number of players determined by the division the game takes place in and the complexity level of the playing teams, as illustrated by the following table:
...
Division & At least 1 Foundation team & 2 Advanced teams\\
%
Small & 4 & 7 \\
Mid   & 3 & 5 \\
Large & 3 & 5 \\
...
The only wireless hardware allowed to be used by the teams are the wireless network interfaces integrated into the robots, and the access points provided by the event organizers.
...
Robot-to-robot communication that violates the rules result in a game scored with 0 goals for the offending team (even when discovered after the game was finished).
Messages are counted towards the limit during the \texttt{ready}, \texttt{set}, \texttt{playing} and \texttt{penalized} states.
