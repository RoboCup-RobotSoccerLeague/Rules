...
\begin{description}
  \item[Unstiff.] This state ensures consistent and safe handling of robots.
    ...
    The robot should move to a safe position and unstiffen all joints.
    ...
    A manual input to the robot will switch it to the \texttt{attended} state as human leave the robot unsupervised on the field.

    \item[Attended.]
    This state is used when a human  enters or is near the field.
    The GameController can active this state from any other state beside \texttt{playing}. 
    Robots in this state must immediately stop any motion, assume a safe posture (e.g. seated or crouched), and remain stationary.
    Robots cannot resume play until the human has exited the field area and the GameController authorizes a transition to a different state.
  ...
  \item[Penalized.] A robot is in this state when it has been \texttt{penalized}.
    The robot must walk to the side of the field according to panelty procedure (\cf \cref{sec:penalty_procedure}).

  \item[Expulsed.] A robot is in this state when it has been \texttt{expulsed}.
    The only allowed movement is standing up after falling or moving to a safe posture (e.g. seated or crouched).
    Any other movement (including turning the head) is disallowed.

  \item[Pickup.] A robot enter this state when a team request the robot to be pickup. 
    It will then walk to his pickup location (\cf \cref{sec:pickuprobots}).
...
The current game state should be displayed on the LED of the robot.
The colors corresponding to the game states are:
\begin{itemize}
  \item \texttt{Unstiff}: Blue-Blinking
  \item \texttt{Initial}: White
  \item \texttt{Attended}: Blinking between Cyan and Red every 254ms
  \item \texttt{Ready}: Blue
  \item \texttt{Set}: Yellow
  \item \texttt{Playing}: Green
  \item \texttt{Penalized}: Red
  \item \texttt{Expulsed}: Purple
  \item \texttt{Pickup}: Orange
  \item \texttt{Finished}: White
\end{itemize}
