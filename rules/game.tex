...
\begin{description}
  \item[Unstiff.] This state ensures consistent and safe handling of robots.
    ...
    A manual input can transition the robot to \texttt{attended} when the robot is left unattended on the field.
...
\texttt{Initial} & White \\
\texttt{Attended} & Alternating Cyan/Red (every 254 ms) \\
\texttt{Ready} & Blue \\
\texttt{Set} & Yellow \\
\texttt{Playing} & Green \\
\texttt{Penalized} & Red \\
